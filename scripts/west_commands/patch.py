...
APPS_BASE = Path(__file__).parent.parent.parent
...
WEST_PATCH_BASE = Path("patches")
WEST_PATCH_YAML = Path("patches") / "patches.yml"
...
class Patch(WestCommand):
    ...
    def get_module_path(self, module_name_or_path):
        ...
        all_modules = zephyr_module.parse_modules(APPS_BASE, self.manifest)
