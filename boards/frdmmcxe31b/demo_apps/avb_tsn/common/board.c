...
void BOARD_InitGPIO(void)
{
    ...
    SIUL2_EnableExtInterrupt(BOARD_SIUL2_BASE, BOARD_USER_BUTTON_EIRQ,
...
{
    return SIUL2_GetExtDmaInterruptStatusFlags(BOARD_SIUL2_BASE) & (1UL << BOARD_USER_BUTTON_EIRQ);
}
...
{
    SIUL2_ClearExtDmaInterruptStatusFlags(BOARD_SIUL2_BASE, 1U << BOARD_USER_BUTTON_EIRQ);
}

void BOARD_InitNVIC(void)
{
    NVIC_SetPriority(EMAC_0_IRQn, configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY + 3);
    NVIC_SetPriority(LPUART_5_IRQn, configLIBRARY_LOWEST_INTERRUPT_PRIORITY + 3);
    NVIC_SetPriority(STM0_IRQn, configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY + 3);
}
