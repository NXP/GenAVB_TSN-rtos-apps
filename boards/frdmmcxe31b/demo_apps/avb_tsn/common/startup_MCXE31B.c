...
extern uint32_t __slow_data_load[];
extern uint32_t __slow_data_start[];
extern uint32_t __slow_data_end[];
...
extern uint32_t __slow_bss_start[];
extern uint32_t __slow_bss_end[];
#else
#error Unsupported toolchain!
#endif
 ...
 */
    ...
    pDataSrc = (uint32_t *)__slow_data_load;
    pDataDest = (uint32_t *)__slow_data_start;
    while (pDataDest < __slow_data_end)
    {
        *pDataDest ++ = *pDataSrc++;
    }
 ...
 */
    ...
    pDataDest = (uint32_t *)__slow_bss_start;
    while (pDataDest < __slow_bss_end)
    {
        *pDataDest++ = 0;
    }
