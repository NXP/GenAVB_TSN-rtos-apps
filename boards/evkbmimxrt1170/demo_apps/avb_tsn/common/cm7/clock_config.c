...
void BOARD_InitEnetClock(void)
{
    ...
    /* SYS_PLL1, used primarily by the ENET module. It has a fixed frequency of 1GHz. */
    ...
    /* Generate 125M root clock for Enet1 */
    /* 1G / 2 / 4 = 125M */
    rootCfg.mux = kCLOCK_ENET1_ClockRoot_MuxSysPll1Div2;
    rootCfg.div = 4U;
    CLOCK_SetRootClock(kCLOCK_Root_Enet1, &rootCfg);

    /* Generate 200M root clock for Enet Timer1 */
    /* 1G / 5 / 1 = 200M */
...
static void IOMUXC_SelectENET1GClock(void)
{
    IOMUXC_GPR->GPR5 |= IOMUXC_GPR_GPR5_ENET1G_RGMII_EN_MASK; /* bit1:iomuxc_gpr_enet_clk_dir */
                                                              /* bit0:GPR_ENET_TX_CLK_SEL(internal or OSC) */
}

void BOARD_InitEnet1GClock(void)
{
    clock_root_config_t rootCfg = {0};
    ...
    /* Generate 125M root clock for Enet2 (1G) */
    /* 1G / 2 / 4 = 125M */
    rootCfg.mux = kCLOCK_ENET2_ClockRoot_MuxSysPll1Div2; // == 4
    rootCfg.div = 4U;
    CLOCK_SetRootClock(kCLOCK_Root_Enet2, &rootCfg);

    /* Generate 200M root clock for Enet Timer2 */
    /* 1G / 5 / 1 = 200M */
    rootCfg.mux = kCLOCK_ENET2_ClockRoot_MuxSysPll1Div5;
    rootCfg.div = 1U;
    CLOCK_SetRootClock(kCLOCK_Root_Enet_Timer2, &rootCfg);
...
#error BOARD_USE_ENET_QOS is not defined
#endif
...
void BOARD_InitMediaClock(void)
{
    clock_root_config_t rootCfg = {0};
    /**
     * AUDIO PLL: output frequency = (Fref(24Mhz) * (DIV_SELECT + NUM / DENOM)) / (1 << POST_DIV_SEL)
     * freq = (24MHz * (32 + 153600000/200000000)) / 2 = 786432000 / 2 = 393216000 Hz
     */
    const clock_audio_pll_config_t audio_pll_config = {
        .loopDivider = 32,        /* DIV_SELECT: Valid range: 27~54. */
        .numerator   = 153600000,  /* NUM: 30 bit numerator of fractional loop divider. */
        .denominator = 200000000, /* DENOM: 30 bit denominator of fractional loop divider */
        .postDivider = 1          /* Post Divide Select */
    };

    CLOCK_InitAudioPll(&audio_pll_config);

    /**
     * lpi2c5_clk_root: Audio codec control interface
     * source: OSC_24M
     * freq: source / 1 = 24MHz
     */
    ...
    /**
     * sai1_clk_root, sai2_clk_root, sai3_clk_root, sai4_clk_root
     * source: PLL_AUDIO
     * freq: Audio Pll output / div = 393216000 / 32 = 12288000
     */
    rootCfg.div = 32;
    ...
    /* Configure media clock recovery gpt timer using Audio PLL divided by 2 */
    rootCfg.div = 2;
#if BOARD_USE_ENET_QOS
    rootCfg.mux = kCLOCK_GPT3_ClockRoot_MuxAudioPllOut;
    CLOCK_SetRootClock(kCLOCK_Root_Gpt3, &rootCfg);
    /* connect GPT3 input capture channel 1 source to ENET_QOS ptp_pps_o[1] */
    IOMUXC_GPR->GPR24 |= IOMUXC_GPR_GPR24_GPT3_CAPIN1_SEL(1);
#else
    rootCfg.mux = kCLOCK_GPT2_ClockRoot_MuxAudioPllOut;
    CLOCK_SetRootClock(kCLOCK_Root_Gpt2, &rootCfg);
    /* connect GPT2 input capture channel 2 source to ENET1G_1588_EVENT1_OUT */
    IOMUXC_GPR->GPR23 |= IOMUXC_GPR_GPR23_GPT2_CAPIN2_SEL(1);
#endif /* BOARD_USE_ENET_QOS */
#endif /* CONFIG_APP_MCR */

    /* Enable Sai Mclk Output */
    IOMUXC_GPR->GPR0 |= IOMUXC_GPR_GPR0_SAI1_MCLK_DIR(1);
}
