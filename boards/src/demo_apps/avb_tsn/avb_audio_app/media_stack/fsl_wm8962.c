...
status_t WM8962_Init(wm8962_handle_t *handle, const wm8962_config_t *config)
{
    ...
    uint32_t mclk = config->format.mclk_HZ;
    ...
    /* Enable MIC BIAS, enable left and right ADC, enable left and right PGA and mixer,
       Enable bias current generator, VMID 2 x 5k divider (for fast start-up) */
    ...
    /*
    * Disable left input mixer volume
    */
    WM8962_CHECK_RET(WM8962_WriteReg(handle, WM8962_LINPATH, 0x000), ret);
    /*
    * Update right input mixer volume
    */
    WM8962_CHECK_RET(WM8962_WriteReg(handle, WM8962_RINPATH, INPGAR_MIXINR_VOL), ret);
    ...
    /* input PGA volume */
    WM8962_CHECK_RET(WM8962_WriteReg(handle, WM8962_LINVOL, IN_VU | INPGAL_MUTE_BIT), ret);
    WM8962_CHECK_RET(WM8962_WriteReg(handle, WM8962_RINVOL, IN_VU | INR_VOL), ret);
    /* Headphone volume, +6dB */
    WM8962_CHECK_RET(WM8962_WriteReg(handle, WM8962_LOUT1, HPOUTL_VU_BIT | HPOUTL_VOL), ret);
    WM8962_CHECK_RET(WM8962_WriteReg(handle, WM8962_ROUT1, HPOUTR_VU_BIT | HPOUTR_VOL), ret);
    /*
     * Digital audio interface data control
     * Word length: 32 bits
     * Format: Left Justified
     */
    WM8962_CHECK_RET(WM8962_WriteReg(handle, WM8962_IFACE0, 0xD), ret);
...
status_t WM8962_SetDataRoute(wm8962_handle_t *handle, const wm8962_route_config_t *route)
{
    ...
    WM8962_CHECK_RET(WM8962_WriteReg(handle, WM8962_INPUT_MIXER_1, 3U), ret); /* Left and right input mixer enable*/
...
status_t WM8962_WriteReg(wm8962_handle_t *handle, uint16_t reg, uint16_t val)
{
    ...
    data[0] = (reg >> 8) & (0xff); // register address MSB
    data[1] = reg & 0xff;          // register address LSB
    data[2] = (val >> 8) & (0xff); // data MSB
    data[3] = val & 0xff;          // data LSB

    retval = handle->send_data(handle->instance, data, 4);
...
status_t WM8962_ReadReg(wm8962_handle_t *handle, uint16_t reg, uint16_t *val)
{
    ...
    uint8_t read_reg[2]; // register address to read data

    read_reg[0] = (reg >> 8) & (0xff); // register address MSB
    read_reg[1] = reg & 0xff;          // register address LSB

    retval = handle->receive_data(handle->instance, read_reg, 2U, (uint8_t *)&readValue, 2U);
    if (retval == kStatus_Success)
    {
