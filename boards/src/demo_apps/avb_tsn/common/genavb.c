...
static int avdecc_config_init(struct avdecc_config *avdecc_config)
{
    ...
    if (!aem_id) {
        /* Make the AVDECC stack use the static entity. */
        avdecc_config->entity_cfg[0].num_aems = 0;
    } else {
        ...
        /* Only support a single AEM configurtion per entity for now. */
        avdecc_config->entity_cfg[0].aem[0] = aem_entity;
        avdecc_config->entity_cfg[0].num_aems = 1;
    }
