...
static void memcpy_aligned32(uint8_t *dst, const uint8_t *src, unsigned int len)
{
    unsigned int len32;
...
static void memcpy_aligned(uint8_t *dst, const uint8_t *src, unsigned int len)
{
...
static void memcpy_unaligned(uint8_t *dst, const uint8_t *src, unsigned int len)
{
...
static void *__memcpy(uint8_t *dst, const uint8_t *src, size_t len)
{
    switch (len) {
    case 0:
        break;
...
{
    uint8_t *_dst = dst;
    uint32_t word = c & 0xff;
    uint64_t dword;

    word = word | (word << 8) | (word << 16) | (word << 24);
    dword = word | ((uint64_t)word << 32);

    if (((unsigned long)_dst & 0x1) && (len >= 1)) {
        *(uint8_t *)_dst = dword;
        len--;
        _dst++;
    }

    if (((unsigned long)_dst & 0x2) && (len >= 2)) {
        *(uint16_t *)_dst = dword;
        len -= 2;
        _dst += 2;
    }

    if (((unsigned long)_dst & 0x4) && (len >= 4)) {
        *(uint32_t *)_dst = dword;
        len -= 4;
        _dst += 4;
    }

    for (; len >= 8; len -= 8, _dst += 8)
        *(uint64_t *)_dst = dword;
