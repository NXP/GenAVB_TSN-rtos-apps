...
#define MAX_FAST_LOOP_EXEC    (3 * ((M1_PWM_FREQ + M1_SPEED_LOOP_FREQ - 1) / M1_SPEED_LOOP_FREQ)) /* Equivalent to ceil(M1_PWM_FREQ / M1_SPEED_LOOP_FREQ) */
...
#ifdef CONFIG_APP_MOTOR_ENCODER_ABS_POSITION
RAM_FUNC_LIB
static void M1_StateRunAlignIndexSlow(void);

RAM_FUNC_LIB
static void M1_StateRunAlignIndexFast(void);

RAM_FUNC_LIB
static void M1_TransRunAlignIndex(void);
#endif /* CONFIG_APP_MOTOR_ENCODER_ABS_POSITION */
...
static const pfcn_void_void s_M1_STATE_RUN_TABLE_FAST[8] = {
    M1_StateRunCalibFast, M1_StateRunReadyFast,     M1_StateRunAlignFast,  M1_StateRunStartupFast,
    M1_StateRunSpinFast,  M1_StateRunFreewheelFast, NULL, M1_StateRunAlignIndexFast};

/*! @brief Application sub-state function field - slow */
static const pfcn_void_void s_M1_STATE_RUN_TABLE_SLOW[8] = {
    M1_StateRunCalibSlow, M1_StateRunReadySlow,     M1_StateRunAlignSlow,  M1_StateRunStartupSlow,
    M1_StateRunSpinSlow,  M1_StateRunFreewheelSlow, NULL, M1_StateRunAlignIndexSlow};
...
#ifdef CONFIG_APP_MOTOR_ENCODER_ABS_POSITION
static uint32_t ui32AlignIndexCounter = 0;
#endif
...
static void M1_StateInitFast(void)
{
    ...
    g_sM1Drive.bInitOver = FALSE;
    g_sM1Drive.sFocPMSM.bEnableCurrentPI = TRUE;
    g_sM1Drive.ui32counterFastLoopExec = 0;
    g_sM1Drive.ui32CounterSlowLoopMissed = 0;
...
static void M1_StateRunAlignFast(void)
{
    ...
    if (--g_sM1Drive.ui16CounterState == 0U)
    {
        ...
        {
#ifdef CONFIG_APP_MOTOR_ENCODER_ABS_POSITION
            /* Transition to the RUN kRunState_AlignIndex sub-state */
            M1_TransRunAlignIndex();
#else
...
static void M1_StateRunSpinFast(void)
{
    ...
    switch (g_sM1Drive.eControl)
    {
        ...
        default:
            ...
            if (g_sM1Drive.ui32counterFastLoopExec > MAX_FAST_LOOP_EXEC) {
                g_sM1Drive.sFocPMSM.bEnableCurrentPI = FALSE;
                g_sM1Drive.iq_external_command = 0.0;
                g_sM1Drive.sFocPMSM.sIDQReq.fltQ = 0.0;
                g_sM1Drive.ui32CounterSlowLoopMissed++;
            }
            else
            {
               g_sM1Drive.sFocPMSM.bEnableCurrentPI = TRUE;
            ...
            /* Add one fast loop exec */
            g_sM1Drive.ui32counterFastLoopExec++;
...
static void M1_StateRunSpinSlow(void)
{
    ...
    if (g_sM1Drive.eControl == kControlMode_PositionFOC)
    {
        ...
        /* Reset counter of fast loop executions */
        g_sM1Drive.ui32counterFastLoopExec = 0;

        if (g_sM1Drive.bNetworkingCommand)
        {
            g_sM1Drive.sFocPMSM.sIDQReq.fltQ = g_sM1Drive.iq_external_command;
        }
        else
        {
...
static void M1_TransRunAlignSpin(void)
{
    g_sM1Drive.bInitOver = TRUE;

#ifdef CONFIG_APP_MOTOR_ENCODER_ABS_POSITION
    g_sM1Enc.i16RevCounterCorrection = 0;
...
static void M1_TransRunAlignIndex(void)
{
    ...
    /* Make one revolution to find the index */
    g_sM1Drive.sPosition.a32PositionCmd = g_sM1Drive.sPosition.a32Position + ACC32(1.0);

    /* To switch to the RUN SPIN sub-state */
    g_eM1StateRun = kRunState_AlignIndex;

    /* Alignment duration to ensure we let time to reach index position */
    ui32AlignIndexCounter = M1_ALIGN_DURATION;

    /* Enable index interrupt */
    M1_MCDRV_QD_EN_INDEX_INT(&g_sM1Enc);
}

static void M1_StateRunAlignIndexSlow(void)
{
    /* Go to spin state if index found and alignment time elapsed */
    if (ui32AlignIndexCounter-- == 0) {
        if (g_sM1Enc.bIndexFound)
            M1_TransRunAlignSpin();
        else
            // If we didn't find the index, we retry
            M1_TransRunAlignIndex();
    }
...
static void M1_StateRunAlignIndexFast(void)
{
    ...
    if (M1_MCDRV_QD_GET_INDEX_INT_STATUS(&g_sM1Enc) && !g_sM1Enc.bIndexFound) {
        M1_MCDRV_QD_CLEAR_INDEX_INT_STATUS(&g_sM1Enc);
        M1_MCDRV_QD_CLEAR(&g_sM1Enc);
        g_sM1Drive.sPosition.a32PositionCmd = 0.0;
        // Here, we do not reset software revolutions in order not to hide the discontinuity
        g_sM1Enc.i32PosAbsoluteEnc = 0;
        g_sM1Enc.i16PosRelativeEnc = 0;
        g_sM1Enc.bIndexFound = TRUE;
    }
}
...
void M1_SetPosition(acc32_t a32PositionCmdDemo)
{
    ...
    {
        /* Set position */
        g_sM1Drive.sPosition.a32PositionCmd = a32PositionCmdDemo + g_sM1Drive.sPosition.a32IndexPos;
    }
    else
    {
        /* Set zero position */
...
/*!
 * @brief Get real speed filtered of the motor in float value
 *
 * @param void  No input parameter
 *
 * @return float_t Float value of the current speed
 */
float_t M1_GetRealSpeed(void)
{
    /* return speed */
    return g_sM1Drive.sSpeed.fltSpeedFilt * g_fltM1speedAngularScale;
}

/*!
* @brief Get position of the motor in float value
*
* @return actual position
*/
float_t M1_GetPositionFloat(void)
{
    return ((float) g_sM1Enc.i32PosAbsoluteEnc / g_sM1Enc.ui32NbEncoderCounts);
}

/*!
* @brief Get last position index
*
* @return actual position
*/
uint32_t M1_GetLastIndexPosition(void)
{
#ifdef CONFIG_APP_MOTOR_ENCODER_ABS_POSITION
    return g_sM1Enc.ui32LastIndexPosEnc;
#else
    return 0;
#endif
}
...
/*!
* @brief Get counter of position jumps
*
* @return counter of position jumps
*/
uint32_t M1_GetCounterRevJumps(void)
{
#ifdef CONFIG_APP_MOTOR_ENCODER_ABS_POSITION
    return g_sM1Enc.ui32CountRevJumps;
#else
    return 0;
#endif
}
