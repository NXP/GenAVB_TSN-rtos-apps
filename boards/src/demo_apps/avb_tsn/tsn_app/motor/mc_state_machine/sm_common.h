...
typedef struct _mcdef_pmsm_t
{
    ...
    bool_t bInitOver;                              /* Initialization over flag */
    bool_t bNetworkingCommand;                     /* Extern control state */
    float_t iq_external_command;                   /* Iq Command received from the network */
    uint32_t ui32counterFastLoopExec;              /* Counter of fast loop executions between multiple slow loop executions*/
    uint32_t ui32CounterSlowLoopMissed;            /* Counter incremented when the number of fast loop execution has exceeded its maximum between two slow loops */
} mcdef_pmsm_t;
