 ...
 ******************************************************************************/
#ifdef MCDRV_QDDEBUG
static void MCDRV_QdDebug(mcdrv_qd_enc_t *this, int16_t rev, int16_t pos)
{
    static int cnt = 0;
...
#else
static inline void MCDRV_QdDebug(mcdrv_qd_enc_t *this, int16_t rev, int16_t pos){};
#endif
...
void MCDRV_QdEncGet(mcdrv_qd_enc_t *this)
{
    ...
    */
       ...
       if (!this->bIndexFound) {
           ...
           this->i32PosAbsoluteEnc = 0;
           this->i16PosRelativeEnc = 0;
           this->bIndexFound = TRUE;
        }
   }

   if (this->bIndexFound)
       this->i32PosAbsoluteEnc = RevCounter * this->ui32NbEncoderCounts + this->i16PosRelativeEnc;
   ...
   this->f16RevCounter = (frac16_t)(this->i16RevCounter);
...
{
    ENC_DisableInterrupts(this->pui32QdBase, kENC_INDEXPulseInterruptEnable);
}
...
{
    ENC_EnableInterrupts(this->pui32QdBase, kENC_INDEXPulseInterruptEnable);
    this->pui32QdBase->CTRL |= (1 << ENC_CTRL_XIRQ_SHIFT);
}
...
{
    return (this->pui32QdBase->CTRL & (1 << ENC_CTRL_XIRQ_SHIFT));
}
