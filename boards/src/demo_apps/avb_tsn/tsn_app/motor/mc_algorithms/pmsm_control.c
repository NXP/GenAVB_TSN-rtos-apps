...
void MCS_PMSMFocCtrl(mcs_pmsm_foc_t *psFocPMSM)
{
    ...
    if (psFocPMSM->bCurrentLoopOn)
    {
        if (psFocPMSM->bEnableCurrentPI)
        {
            ...
            psFocPMSM->sIdPiParams.fltLowerLim =
                MLIB_MulNeg_FLT(psFocPMSM->fltDutyCycleLimit, psFocPMSM->fltUDcBusFilt);
        ...
        {
            psFocPMSM->sUDQReq.fltD = 0.0;
            psFocPMSM->sUDQReq.fltQ = 0.0;
            psFocPMSM->sIDQReq.fltD = 0.0;
            psFocPMSM->sIDQReq.fltQ = 0.0;
            psFocPMSM->sIDQ.fltD    = 0.0;
            psFocPMSM->sIDQ.fltQ    = 0.0;
            psFocPMSM->sIDQ.fltD    = 0.0;
            psFocPMSM->sIDQ.fltQ    = 0.0;
        }
    }
