...
/* Region 2 setting */
    ...
    /* Region 4 setting (ITCM): Memory with Normal type, not shareable, outer/inner write back */
    ...
    /* Region 6 setting (DTCM): Memory with Normal type, not shareable, outer/inner write back */
    MPU->RBAR = ARM_MPU_RBAR(6, 0x20000000U);
    MPU->RASR = ARM_MPU_RASR(0, ARM_MPU_AP_FULL, 0, 0, 1, 1, 0, ARM_MPU_REGION_SIZE_512KB);

    /* Region 7 setting (OCRAM) */
...
static void BOARD_EnetPhyReset(void)
{
    ...
    GPIO_PinInit(BOARD_ENET_INT_GPIO, BOARD_ENET_INT_PIN, &gpio_config);
    /* pull up the ENET_INT before RESET. */
    GPIO_WritePinOutput(BOARD_ENET_INT_GPIO, BOARD_ENET_INT_PIN, 1);
